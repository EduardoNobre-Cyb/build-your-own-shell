...
def complete(text, state):
    if state == 0:
        ...
        if (
            len(line_parts) >= 1
            and line_parts[0] == "cd"
            and (len(line_parts) > 1 or line_buffer.endswith(" "))
        ):
            # Complete with directories
            search_text = text
            if text.startswith("~"):
                search_text = os.path.expanduser(text)

            # Determine directory to search and prefix
            if "/" in search_text:
                dir_path = os.path.dirname(search_text)
                prefix = os.path.basename(search_text)
            else:
                dir_path = "."
                prefix = search_text
            ...
            try:
                matches = []
                list_dir = os.path.expanduser(dir_path) if dir_path else "."
                for item in os.listdir(list_dir):
                    ...
                    if item.startswith(prefix) and os.path.isdir(item_path):
                        if dir_path and dir_path != ".":
                            match = os.path.join(dir_path, item)
                        else:
                            match = item
                        # Add / for directories
                        if text.startswith("~") and not match.startswith("~"):
                            # Restore ~ prefix
                            home = os.path.expanduser("~")
                            full_match = os.path.join(list_dir, item)
                            if full_match.startswith(home):
                                match = "~" + full_match[len(home) :]
                        matches.append(match)
                complete.matches = sorted(matches)
            except:
                complete.matches = []
        else:
            # Complete with commands (builtins + PATH executables)
